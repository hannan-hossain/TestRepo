...
namespace TS.Repositories
{
    public partial class GenericRepository<T> where T : DbContext, IDisposable
    {
        ...
        public void IntiailizeDate<TEntity>(TEntity entity) where TEntity : class
        {
            if (entity != null)
            {
                Context.Entry<TEntity>(entity).Property("DateCreatedUtc").CurrentValue = DateTime.UtcNow;
                Context.Entry<TEntity>(entity).Property("DateModifiedUtc").CurrentValue = DateTime.UtcNow;
            }
        }
    }
}
